...
try {
    if (chrome.offscreen && chrome.offscreen.createDocument) {
        chrome.offscreen.hasDocument().then((has) => {
            if (!has) {
                chrome.offscreen.createDocument({ url: "offscreen.html", reasons: ["BLOBS"], justification: "long-running downloads" });
            }
        }).catch(() => {
            try { chrome.offscreen.createDocument({ url: "offscreen.html", reasons: ["BLOBS"], justification: "long-running downloads" }); } catch (e) {}
        });
    }
} catch (e) {}

function updateCloudRules(force = false) {
    if (!G || !G.RuleSubscriptionUrl) { return; }
    chrome.storage.local.get({ CloudRulesVersion: 0 }, (v) => {
        fetch(G.RuleSubscriptionUrl).then(res => res.json()).then(json => {
            const remoteVersion = json.version || Date.now();
            if (!force && v.CloudRulesVersion >= remoteVersion) { return; }
            const mergeArray = (a = [], b = []) => {
                const exist = new Set(a.map(x => JSON.stringify(x)));
                const add = b.filter(x => !exist.has(JSON.stringify(x)));
                return a.concat(add);
            };
            const sync = {};
            if (Array.isArray(json.Regex)) { sync.Regex = mergeArray(G.OptionLists.Regex, json.Regex); }
            if (Array.isArray(json.Ext)) { sync.Ext = mergeArray(G.OptionLists.Ext, json.Ext); }
            if (Array.isArray(json.Type)) { sync.Type = mergeArray(G.OptionLists.Type, json.Type); }
            if (Array.isArray(json.blockUrl)) { sync.blockUrl = mergeArray(G.OptionLists.blockUrl, json.blockUrl); }
            Object.keys(sync).length && chrome.storage.sync.set(sync);
            chrome.storage.local.set({ CloudRulesVersion: remoteVersion, CloudRulesUpdatedAt: Date.now() });
        }).catch(() => { });
    });
}
...
chrome.runtime.onMessage.addListener(function (Message, sender, sendResponse) {
    ...
    if (Message.Message === "UpdateRules") {
        updateCloudRules(true);
        sendResponse("OK");
        return true;
    }
});

setTimeout(() => { try { updateCloudRules(false); } catch (e) {} }, 8000);
