...
chrome.storage.local.get({ m3u8DownloadState: null }, function (data) {
    const state = data.m3u8DownloadState;
    if (state && typeof state == "object") {
        const text = state.completed ? `${state.success}/${state.total}` : `${state.success ?? 0}/${state.total ?? 0}`;
        $("#TipsFixed").html(text);
    }
});
// 批量插入优化
const _pendingCurrent = [];
const _pendingAll = [];
let _flushTimerCurrent = null;
let _flushTimerAll = null;
function flushCurrent() {
    _flushTimerCurrent = null;
    if (!_pendingCurrent.length) return;
    const frag = document.createDocumentFragment();
    for (const el of _pendingCurrent) { frag.appendChild(el); }
    _pendingCurrent.length = 0;
    $current[0].appendChild(frag);
    UItoggle();
}
function flushAll() {
    _flushTimerAll = null;
    if (!_pendingAll.length) return;
    const frag = document.createDocumentFragment();
    for (const el of _pendingAll) { frag.appendChild(el); }
    _pendingAll.length = 0;
    $all[0].appendChild(frag);
    UItoggle();
}
function scheduleAppendCurrent(html) {
    _pendingCurrent.push(html[0]);
    if (!_flushTimerCurrent) { _flushTimerCurrent = setTimeout(flushCurrent, 16); }
}
function scheduleAppendAll(html) {
    _pendingAll.push(html[0]);
    if (!_flushTimerAll) { _flushTimerAll = setTimeout(flushAll, 16); }
}
            ...
            if (isM3U8(data)) {
                ...
                hls.on(Hls.Events.BUFFER_CREATED, function (event, data) {
                    ...
                    if (data.tracks?.video?.codec && data.tracks.video.codec.startsWith("hvc1")) {
                        mediaInfo.append(`<br><b>${i18n.hevcTip}</b>`);
                    }
                });
                ...
                hls.on(Hls.Events.LEVEL_LOADED, function (event, data) {
                    if (Array.isArray(data.fragments) && data.fragments.some(item => item.encrypted)) {
                        mediaInfo.append(`<br><b>${i18n.encryptedHLS}</b>`);
                    }
                });
            } else if (isMPD(data)) {
                setRequestHeaders(data.requestHeaders, function () {
                    fetch(data.url).then(res => res.text()).then(text => {
                        ...
                        if (cps && cps.length) {
                            mediaInfo.append(`<br><b>${i18n.DRMerror}</b>`);
                        }
                    }).catch(() => { });
                });
            } else if (data.isPlay) {
...
const interval = setInterval(async function () {
    ...
    chrome.runtime.onMessage.addListener(function (Message, sender, sendResponse) {
        ...
        if (Message.Message == "popupAddData") {
            ...
            if (Message.data.tabId == G.tabId) {
                ...
                currentCount++;
                scheduleAppendCurrent(html);
            } else if (allCount) {
                allCount++;
                scheduleAppendAll(html);
            }
            sendResponse("OK");
            return true;
        }
    ...
    const setTheme = (theme) => {
        ...
        const $btns = $('#ThemeSwitcher .button');
        $btns.removeClass('Active');
        $btns.filter(`[data-theme='${theme}']`).addClass('Active');
        chrome.storage.local.set({ uiTheme: theme });
    };
    chrome.storage.local.get({ uiTheme: 'system' }, function (data) {
        setTheme(data.uiTheme);
    });
    $('#ThemeSwitcher .button').click(function () {
        setTheme(this.dataset.theme);
    });

    function passesAdvancedFilter(item) {
        const mimeInput = ($('#mimeInput').val() || '').trim();
        const min = parseInt($('#sizeMin').val());
        const max = parseInt($('#sizeMax').val());
        const mimes = mimeInput ? mimeInput.split(',').map(s => s.trim()).filter(Boolean) : [];
        let ok = true;
        if (mimes.length) {
            const t = item.type || '';
            ok = mimes.some(m => m.endsWith('/*') ? t.startsWith(m.slice(0, -1)) : t === m);
        }
        if (ok && !isNaN(min)) { ok = (item._size ?? 0) >= min; }
        if (ok && !isNaN(max) && max > 0) { ok = (item._size ?? 0) <= max; }
        return ok;
    }
    $('#applyAdvancedFilter').click(function () {
        getData().forEach(function (data) {
            const visible = passesAdvancedFilter(data);
            data.checked = visible && (!!filterExt.get(data.ext));
            if (visible && filterExt.get(data.ext)) {
                data.html.show();
            } else {
                data.html.hide();
            }
    ...
    document.addEventListener('keydown', function (e) {
        ...
        if (k === 'a') { $('#AllSelect').click(); }
        else if (k === 'i') { $('#invertSelection').click(); }
        else if (k === 'f') { $('#openFilter').click(); }
        else if (k === 'd') { $('#DownFile').click(); }
    });
