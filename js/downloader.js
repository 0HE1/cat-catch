...
function start() {
    ...
    const $header = $("<div id=\"downHeader\" class=\"textColor\"></div>");
    const $topBox = $("#downfile").next("section").find(".optionBox");
    ($topBox.length ? $topBox : $("#downfile .optionBox")).prepend($header);
    let lastThread = down.thread;
    $header.text(`thread: ${down.thread}`);
    ...
    down.on('itemProgress', function (fragment, state, receivedLength, contentLength, value) {
        ...
        if (Date.now() - lastEmitted >= 100 && !state) {
            ...
            if (lastThread != down.thread) {
                $header.text(`thread: ${down.thread}`);
                lastThread = down.thread;
            }
            try { chrome.runtime.sendMessage({ Message: "persistDownloadState", data: { type: "single", success: down.success, total: down.total, buffersize: down.buffersize } }); } catch (e) {}
        }
    });
    ...
    down.on('downloadError', function (fragment, error) {
        ...
        const attempts = fragment.retryAttempts || 0;
        itemDOM.get(fragment.index).progressText.html(`${error} (${attempts})`);
