...
if (typeof document !== "undefined" && document.addEventListener) {
    document.addEventListener('DOMContentLoaded', function () {
        chrome.storage.local.get({ uiTheme: 'system' }, function (data) {
            if (data.uiTheme === 'system') {
                document.documentElement.removeAttribute('data-theme');
            } else {
                document.documentElement.setAttribute('data-theme', data.uiTheme);
            }
        });
        const btn = document.getElementById('UpdateRules');
        if (btn) {
            btn.addEventListener('click', function () {
                chrome.runtime.sendMessage({ Message: 'UpdateRules' }, function () { });
            });
        }
    });
}
