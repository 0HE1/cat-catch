class Downloader {
    ...
    init() {
        ...
        this.maxThread = Math.max(2, this.thread);
        this.minThread = Math.min(2, this.thread);
        this.errorStreak = 0;
    ...
    downloader(fragment = null) {
        ...
        if (fragment && fragment.requestHeaders) {
            options.headers = { ...fragment.requestHeaders };
        }
        ...
        fetch(fragment.url, options)
            ...
            .then(buffer => {
                ...
                this.successStreak++;
                this.errorStreak = 0;
                if (this.successStreak >= 10 && this.thread < this.maxThread) {
                    this.thread++;
                    if (this.running < this.thread && this.index < this.fragments.length) {
                        this.downloader();
                    }
                    this.successStreak = 0;
                }
                ...
                fragment.retryAttempts = (fragment.retryAttempts || 0) + 1;
                let maxAttempts = 5;
                let base = 2000;
                if (code === 429) {
                    this.thread = this.minThread;
                    base = 8000;
                } else if (code === 403) {
                    this.thread = Math.max(this.minThread, this.thread - 2);
                    base = 4000;
                } else if (code === 404) {
                    maxAttempts = 2;
                    base = 1000;
                } else if (code >= 500 && code <= 599) {
                    this.thread = Math.max(this.minThread, this.thread - 1);
                    base = 3000;
                } else if (this.errorStreak >= 3 && this.thread > this.minThread) {
                    this.thread--;
                }
                if (fragment.retryAttempts <= maxAttempts) {
                    const jitter = Math.floor(Math.random() * 500);
                    const backoff = Math.min(60000, base * Math.pow(2, fragment.retryAttempts) + jitter);
                    setTimeout(() => { this.downloader(fragment); }, backoff);
                }
            }).finally(() => {
                this.running--;
                // 下载下一个切片
