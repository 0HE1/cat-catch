...
chrome.storage.local.get({ m3u8DownloadState: null }, function (data) {
    const s = data.m3u8DownloadState;
    if (s && typeof s == "object") {
        const text = s.completed ? `${s.success}/${s.total}` : `${s.success ?? 0}/${s.total ?? 0}`;
        $progress.html(text);
    }
});
...
function downloadNew(start = 0, end = _fragments.length) {
    ...
    down.on('allCompleted', async function (buffer) {
        ...
        buttonState("#mergeTs", true);
        try { chrome.runtime.sendMessage({ Message: "persistDownloadState", data: { type: "m3u8", success: down.success, total: down.total, buffersize: down.buffersize, duration: down.duration, completed: true } }); } catch (e) {}
    });
    // 单个项目下载进度
    ...
    down.on('itemProgress', function (fragment, state, receivedLength, contentLength) {
        if (Date.now() - lastEmitted >= 233) {
            ...
            try { chrome.runtime.sendMessage({ Message: "persistDownloadState", data: { type: "m3u8", success: down.success, total: down.total, buffersize: down.buffersize, duration: down.duration } }); } catch (e) {}
        }
    });
    if (fileStream) {
